...
class Buffer():
	...
	def __init__(self, cfg):
		...
		self._num_eps = 0
		self._debug_shapes = getattr(cfg, "debug_buffer_shapes", False)
	...
	def _prepare_batch(self, td):
		...
		# Ensure time-major ordering (T, B, ...); sampled shape is (batch, time, ...)
		if td.batch_size[0] != self.cfg.horizon + 1:
			td = td.transpose(0, 1).contiguous()
		...
		if terminated is not None:
			terminated = td.get('terminated')[1:].contiguous()
			if terminated.dim() == 2:
	...
	def sample(self):
		...
		if self._debug_shapes:
			print(f"[Buffer.sample] raw batch_size: {td.batch_size}")
		td = td.view(-1, self.cfg.horizon+1).permute(1, 0)
		if self._debug_shapes:
			print(f"[Buffer.sample] after view+permute batch_size: {td.batch_size}")

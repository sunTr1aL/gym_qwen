...
class SpeculativeManager:
        ...
        def _speculate_from(self, z_start: torch.Tensor, task=None) -> List[SpeculativeBranch]:
                ...
                beam: List[SpeculativeBranch] = [
                        SpeculativeBranch(z_sequence=[z_start], actions=[], cumulative_reward=torch.zeros(1, device=self.device))
                ]
                        ...
                        for branch in beam:
                                ...
                                for idx in range(actions.shape[0]):
                                        ...
                                        if self.model.transformer_dynamic:
                                                next_z, dyn_cache = self.model.next(
                                                        current_z, act, task, cache=dyn_cache, return_cache=True
                                                )
                                        else:
